<script lang="ts">
  ...
  $: totalPortfolio = inputs.raBalance + inputs.unitTrustBalance;
  $: raPct = totalPortfolio > 0 ? inputs.raBalance / totalPortfolio : 0;
  $: utPct = totalPortfolio > 0 ? inputs.unitTrustBalance / totalPortfolio : 0;
  $: blendedReturn = totalPortfolio > 0
    ? (inputs.raBalance * inputs.raReturnRate + inputs.unitTrustBalance * inputs.unitTrustReturnRate) / totalPortfolio
    : 0;
</script>
  ...
  <InputSection title="Savings">
    ...
    <div class="savings-summary">
      <div class="sum-row total">
        ...
        <span class="mono">{fmtR(totalPortfolio)}</span>
      </div>
      <div class="sum-row">
        <span>Blended return</span>
        <span class="mono">{(blendedReturn * 100).toFixed(1)}%</span>
      </div>
      <div class="sum-divider"></div>
      <div class="sum-row ra">
        <span>RA — tax-sheltered growth</span>
        <span class="mono">{(raPct * 100).toFixed(0)}%</span>
      </div>
      <div class="sum-note">Lump sum tax applies at retirement (1/3 cash)</div>
      <div class="sum-row ut">
        <span>Unit Trusts — after-tax</span>
        <span class="mono">{(utPct * 100).toFixed(0)}%</span>
      </div>
      <div class="sum-note">CGT + dividends tax on growth; contributions already taxed</div>
    </div>
  </InputSection>
